...
class IsControlling(BasePermission):
    """Controllers must be authenticated,
    then have all permissions in the system"""
...
class IsCustomerSalesContactOrDetailsOrReadOnly(BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        if request.method in SAFE_METHODS:
            return True
        return request.user.id == obj.sales_contact.id or \
    ...
    def has_object_permission(self, request, view, obj):
        if request.method in SAFE_METHODS:
            return request.user.position == 'SALES' or \
                   request.user.position == 'CONTROLLING'
        return request.user.id == obj.contract_customer.sales_contact.id or \
               request.user.position == 'CONTROLLING'
...
class IsEventSalesOrControllerOrEventSupport(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        customer_id = request.POST.get('event_customer', False)  # checks
        # the customer related to the event created
        # (only Sales in charge of that customer may create an event for them)
        ...
        if customer_id is not None:
            ...
            if len(event) >= 1:
                return request.user.id == customer[0].sales_contact.id or \
                       ...
                       request.user.id == event[0].event_support_contact.id  # checks
                # the Supportsperson related to the event, for PUT Request purpose
            return request.user.id == customer[0].sales_contact.id or \
                   request.user.position == 'CONTROLLING'
        return request.user.position == 'CONTROLLING'
